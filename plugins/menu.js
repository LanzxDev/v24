...
let handler = async (m, { conn, usedPrefix: _p, __dirname, args }) => {
  ...
  let arrayMenu = ['all', 'anime', 'update', 'maker', 'edukasi', 'news', 'random', 'game', 'xp', 'islamic', 'stiker', 'rpg', 'kerangajaib', 'quotes', 'admin', 'group', 'premium', 'internet', 'anonymous', 'nulis', 'downloader', 'virus', 'tools', 'fun', 'jadian', 'database','quran', 'vote', 'nsfw', 'audio', 'jadibot', 'info', 'owner', 'nocategory']
...
® ${pmenus} OWNER ADALAH DEWA`, rowId: ".donasi", description: `${namebot}`},
	...
	rows: [
	    {title: `⚡ ${pmenus} SPEED BOT`, rowId: ".ping", description: "Menampilkan kecepatan respon ʟᴀɴʙᴏᴛᴢ"},
	    {title: `🙍 ${pmenus} OWNER BOT`, rowId: ".owner", description: "Menampilkan List owner ʟᴀɴʙᴏᴛᴢ"}, 
	]
    },{
	title: `${htki} SUPPORT ${htka}`,
	rows: [
	    {title: `🍀 ${pmenus} SEWA`, rowId: ".sewa", description: "Menampilkan list harga sewa ʟᴀɴʙᴏᴛᴢ"},
	    {title: `🌟 ${pmenus} BUY PREMIUM`, rowId: ".premium", description: "Menampilkan list harga premium"},
	    {title: `💹 ${pmenus} DONASI`, rowId: ".donasi", description: 'Support ʟᴀɴʙᴏᴛᴢ agar lebih fast respon'},
	]
	},{
	title: `${htki} MENU ${htka}`,
	rows: [
	    {title: `💬 ${pmenus} All`, rowId: ".? all", description: "Menampilkan Semua command ʟᴀɴʙᴏᴛᴢ"},
    ...
    {title: `💘 ${pmenus} Jadian`, rowId: ".? jadian", description: "Semoga langgeng sampai nikah ya kak"},
	{title: `🐚 ${pmenus} Kerang`, rowId: ".? kerangajaib", description: "Tanyakan pada ketua club"},
	{title: `📑 ${pmenus} Quotes`, rowId: ".? quotes", description: "Random Inspirasi"},
	{title: `⛩️ ${pmenus} Anime`, rowId: ".? anime", description: "Kamu wibu ya bang?"},
	{title: `🔞 ${pmenus} Nsfw`, rowId: ".? nsfw", description: "Ih, Sangean"},
    ...
    {title: `➖ ${pmenus} Virus`, rowId: ".? virus", description: "Gunakan fitur ini dengan bijak"},
...
*U S E R  I N F O*
☯︎ *ɴᴀᴍᴇ:* ${usrs.registered ? usrs.name : conn.getName(m.sender)}
☯︎ *ᴛᴀɢs:* @${m.sender.split`@`[0]}
☯︎ *sᴛᴀᴛᴜs:* ${m.sender.split`@`[0] == nomorown ? 'Developer' : (usrs.premiumTime >= 1 ? 'Premium User' : 'Free User')}
☯︎ *ᴘʀᴇᴍɪᴜᴍ:* ${usrs.premiumTime > 1 ? 'Yes': 'No'}
...
const listMessage = {
  text: tek,
  footer: '🌱 *Note:* _Jika menemukan bug, error atau kesulitan dalam penggunaan silahkan laporkan/tanyakan kepada Owner_',
...
function ucapan() {
  ...
  let res = "Oyasumi Oni-Chan >< ☀️"
  if (time >= 4) {
    res = "Ohayou Oni-Chan >< 🌄"
  }
  if (time >= 10) {
    res = "Konnichiwa Oni-Chan >< ☀️"
  }
  if (time >= 15) {
    res = "Konnichiwa Oni-Chan >< 🌇"
  }
  if (time >= 18) {
    res = "Oyasumi Oni-Chan >< 🌙"
  }
  return res
}
